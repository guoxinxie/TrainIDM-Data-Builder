...
# 增加项
class OriginalImageJSONTask_AutoTest_Qwen2_5_vl(ScreenshotMobileTask_AutoTest):
    """
    继承自 ScreenshotMobileTask_AutoTest 以利用其现有的初始化流程。
    """

    def get_agent(self):
        """
        初始化并返回 OriginalImageJSONVisionTask 实例。
        确保将 self.llm_agent 正确传递给基类构造函数中的 'agent' 参数。
        """
        return OriginalImageJSONVisionTask_Qwen2_5_vl(
            instruction=self.instruction,
            controller=self.controller,
            page_executor=self.page_executor,
            agent=self.llm_agent,  # 将模型实例传递给关键字 'agent'
            record=self.record,
    ...
    def get_executor(self):
        """
        初始化并返回 VisionExecutor 实例。
        VisionExecutor 能够处理带有控件列表（elem_list）的页面，
        这对于将模型生成的坐标映射到具体的控件索引至关重要。
        """
        return VisionExecutor(self.controller, self.config)


class OriginalImageJSONTask_AutoTest_Qwen3_vl(ScreenshotMobileTask_AutoTest):
    """
    继承自 ScreenshotMobileTask_AutoTest 以利用其现有的初始化流程。
    """
    ...
    def get_agent(self):
        ...
        确保将 self.llm_agent 正确传递给基类构造函数中的 'agent' 参数。
        """
        return OriginalImageJSONVisionTask_Qwen3_vl(
            instruction=self.instruction,
            controller=self.controller,
            page_executor=self.page_executor,
            agent=self.llm_agent,  # 将模型实例传递给关键字 'agent'
