...
class OpenRouterAgent(OpenAIAgent):
    def __init__(
            ...
            temperature: float = 0,
            site_url: str = 'https://androidlab.github.io',
            site_name: str = 'AndroidLab',
            **kwargs
    ) -> None:
        
        # --- 核心修改 ---
        # 1. 先准备好 OpenRouter 需要的特殊 headers
        openrouter_headers = {
            "HTTP-Referer": site_url,
            "X-OpenRouter-Title": site_name,
        }
        
        # 2. 在创建 OpenAI 客户端时，通过 `default_headers` 参数一次性传入
        self.client = OpenAI(
            api_key=api_key, 
            base_url=api_base,
            default_headers=openrouter_headers  # <--- 在这里传入
        )
        
        # 3. 初始化其他属性 (与父类 OpenAIAgent 保持一致)
        self.model_name = model_name
        self.max_new_tokens = max_new_tokens
        self.temperature = temperature
        self.top_p = 0.7  # 假设一个默认值，或者从 kwargs 获取
        self.kwargs = kwargs
        self.name = "OpenRouterAgent"
        
        # 4. 删除旧的、会报错的赋值语句
        # self.client.default_headers = { ... } # <--- 这行已删除
