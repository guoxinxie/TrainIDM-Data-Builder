...
CSV_PATH = PROJECT_ROOT / "filtered_metadata_2nd" / "split.csv"
TRACE_DIR = PROJECT_ROOT / "mv_trace_en"
AGENT_NAME = "remote_vlm"
OUTPUT_DIR = EVAL_DIR / "outputs"
COMPARISON_CSV_PATH = OUTPUT_DIR / "eval_all_samples_comparison_outputs.csv"
ERROR_LOG_PATH = OUTPUT_DIR / "eval_all_samples_agent_errors.log"
# Set to an integer (e.g. 1000) to cap samples, or None for all selected samples.
MAX_SAMPLES = None
# Seed used when randomly selecting MAX_SAMPLES from all valid selected samples.
# Only takes effect when MAX_SAMPLES is a positive integer.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate agent on valid samples from split.csv (train+test by default)."
    )
    parser.add_argument(
        "-n",
        "--max-samples",
        ...
        dest="max_samples",
        default=None,
        help="Maximum number of valid samples to evaluate. Default: use MAX_SAMPLES in script.",
    )
    parser.add_argument(
        "-w",
        "--max-workers",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Random seed for selecting --max-samples from all valid samples. "
...
def extract_valid_true_samples(
    ...
    """Read split.csv, keep rows with valid=TRUE and selected splits, optional random sampling."""
    ...
    with open(csv_path, "r", newline="", encoding="utf-8") as f:
        ...
        for idx, row in enumerate(reader):
            if str(row.get("valid", "")).strip().lower() != "true":
                continue
            sample_split = str(row.get("split", "")).strip().lower()
            if include_splits is not None and sample_split not in include_splits:
                continue
...
if __name__ == "__main__":
    ...
    effective_max_samples = (
        args.max_samples if args.max_samples is not None else MAX_SAMPLES
    )
    ...
    effective_include_splits = set(INCLUDED_SPLITS)
    effective_csv_path = CSV_PATH.resolve()
    effective_trace_dir = TRACE_DIR.resolve()

    if not effective_csv_path.exists():
        raise FileNotFoundError(f"split CSV not found: {effective_csv_path}")
    if not effective_trace_dir.exists():
        raise FileNotFoundError(f"trace dir not found: {effective_trace_dir}")

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    is_sampling_mode = isinstance(effective_max_samples, int) and effective_max_samples > 0
    should_reuse_cache = COMPARISON_CSV_PATH.exists() and not is_sampling_mode
    ...
    if should_reuse_cache:
        ...
        print(f"Split CSV: {effective_csv_path}")
        print(f"Trace dir: {effective_trace_dir}")
        print(f"Included splits: {sorted(effective_include_splits)}")
        print(f"Max samples: {effective_max_samples}")
        print(f"Sample seed: {effective_sample_seed} (ignored when max samples is None)")
        ...
        for sample in samples:
            split_name = sample.get("split", "") or "unknown"
        ...
        print(f"Trace dir: {effective_trace_dir}")
        print(f"Included splits: {sorted(effective_include_splits)}")
        print(f"Loaded split counts: {split_counts}")
        print(f"Max samples: {effective_max_samples}")
