...
OUTPUT_JSON = TRAIN_DIR / "split_train_qwen3vl_sft.json"
TARGET_SPLIT = "train"
...
def parse_bbox_center(action_text, image_path):
    ...
    x = int(round(x_pixel / width * 1000))
    y = int(round(y_pixel / height * 1000))

    # Qwen3-VL normalized coordinates should stay in [0, 1000].
    x = max(0, min(x, 1000))
    y = max(0, min(y, 1000))
