...
- **AI驱动的逻辑验证**：使用强大的视觉语言模型（可配置，如 Google Gemini）对 UI 的因果关系进行复杂的推理判断。
- **精细化的错误分类**：将 UI 转换分为有效（`none`）、无效（`accidental_touch`）或API调用失败等类别，并附带模型给出的详细文字解释。
- **断点续跑**：自动检测输出 CSV 文件中已处理过的截图对并跳过，使您能够随时中断和恢复长时间运行的任务，无需从头开始。
- **健壮的API调用机制**：内置自动重试与延迟机制，能够优雅地处理临时的网络波动或API错误。
- **专为大规模数据集设计**：能够高效遍历包含数千个交互轨迹的复杂嵌套目录结构。
- **结构化结果输出**：生成一个内容清晰、易于解析的 CSV 文件，汇总所有分析结果。
